...
async def get_url(page, amount, product):
    return f"https://www.maxidom.ru/catalog/{product}/?amount={amount}&PAGEN_2={page}"


async def get_response(product, page=1, amount=30):
    cookies = {
    ...
    response = requests.get(
        await get_url(page, amount, product),
        cookies=cookies,
        headers=headers,
    )
...
async def get_max_pages(product, manager):
    soup = BeautifulSoup((await get_response(product)).content, "lxml")
    count = int(soup.find("div", class_="lvl2__content-nav-numbers-number").find_all("a")[-1].text)
    await manager.nc.publish("parsing", f"Максимум страниц: {count}".encode())
    return count


async def parser_maxidom(product, page_count, manager):
    products = []
    page_count = min(await get_max_pages(product, manager), page_count)

    for page_number in range(1, page_count + 1):
        soup = BeautifulSoup((await get_response(product, page_number)).content, "lxml")
        ...
        await manager.nc.publish("parsing", f"Готова страница {page_number}/{page_count}".encode())
